...
namespace UPCardTool
{
    public class FileSystem
    {
        ...
        public static FileSystem Assets = new FileSystem("mu3_Data\\StreamingAssets\\GameData\\A000\\");
        ...
        public T LoadJson<T>(string fileName)
        {
            ...
            // return NetPacketSerialize.fromJson<T>(File.ReadAllText($"{m_basePath}\\{fileName}"));
        }
        ...
        public void SaveJson<T>(string fileName, T data)
        {
            ...
            // File.WriteAllText($"{m_basePath}\\{fileName}", NetPacketSerialize.toJson<T>(data));
        }
        ...
        public string[] GetFiles(string folder, string extention)
        {
            return Directory.GetFiles($"{m_basePath}\\{folder}", $"*.{extention}", SearchOption.AllDirectories);
        }
    }
}